...
- 在浏览器内合并为一个 `.ts` 文件后保存
- 多码率流提供**清晰度下拉框**，可选具体档位；默认自动选择最高画质
- 支持 **AES-128 加密流**的自动解密（含密钥轮换）、`EXT-X-BYTERANGE` 分片与独立音轨（`EXT-X-MEDIA`）合并
- 直播流（无 `ENDLIST`）会明确提示「仅能下载当前播放窗口的 N 个分片」，不会静默保存残片
- 单文件超过 1.5 GB 时会中止并说明原因，避免浏览器内存溢出
