...
// ---------------------------------------------------------------
// pickVariantForMemoryLimit：音轨合并 + 超内存上限时改选预合并变体
// ---------------------------------------------------------------

const MERGE_MASTER = {
  audioRenditions: [{ groupId: 'aud', uri: 'https://cdn.example.com/audio.m3u8' }],
  isMaster: true,
  variants: [
    { audioGroupId: 'aud', bandwidth: 8000000, label: '2160p', url: 'https://cdn.example.com/2160.m3u8' },
    { audioGroupId: 'aud', bandwidth: 4000000, label: '1080p', url: 'https://cdn.example.com/1080.m3u8' },
    { bandwidth: 2500000, label: '720p', url: 'https://cdn.example.com/720-muxed.m3u8' },
    { bandwidth: 1200000, label: '480p', url: 'https://cdn.example.com/480-muxed.m3u8' },
  ],
};

test('pickVariantForMemoryLimit 未超上限时不改选', () => {
  const mod = loadModule();
  const variant = MERGE_MASTER.variants[0];
  const result = mod.pickVariantForMemoryLimit(MERGE_MASTER, variant, {
...
test('pickVariantForMemoryLimit 超上限且需要音轨合并时改选码率最高的预合并变体', () => {
  const mod = loadModule();
  const result = mod.pickVariantForMemoryLimit(MERGE_MASTER, MERGE_MASTER.variants[0], {
    estimatedBytes: 2.4 * 1024 * 1024 * 1024,
    limitBytes: 2 * 1024 * 1024 * 1024,
  });

  assert.equal(result.changed, true);
  assert.equal(result.muxed.label, '720p');
  assert.equal(result.variant.label, '720p');
});

test('pickVariantForMemoryLimit 已选预合并变体时不改选', () => {
  const mod = loadModule();
  const muxed = MERGE_MASTER.variants[2];
  const result = mod.pickVariantForMemoryLimit(MERGE_MASTER, muxed, {
    estimatedBytes: 2.4 * 1024 * 1024 * 1024,
    limitBytes: 2 * 1024 * 1024 * 1024,
  });

  assert.equal(result.changed, false);
  assert.equal(result.variant, muxed);
});

test('pickVariantForMemoryLimit 没有预合并变体时保持原选择（由上层按上限中止）', () => {
  const mod = loadModule();
  const master = {
    audioRenditions: MERGE_MASTER.audioRenditions,
    isMaster: true,
    variants: MERGE_MASTER.variants.slice(0, 2),
  };
  const result = mod.pickVariantForMemoryLimit(master, master.variants[0], {
    estimatedBytes: 2.4 * 1024 * 1024 * 1024,
    limitBytes: 2 * 1024 * 1024 * 1024,
  });
