...
- 页面无侵入：不向网页注入可见悬浮 UI，识别结果和进度集中在扩展 Popup。
...
| 1.8.0 | 2026-05-28 | 移除页面浮动按钮与浮动弹窗，识别视频列表和下载进度统一由 Popup 展示；页面侧任务通过统一进度消息上报。 |
