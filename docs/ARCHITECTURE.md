...
| 1.17.31 | 2026-09-25 | 回答现场「流体积超过 2048 MB 上限…已中止下载」是否预期，并把它从"终点"改成"自动绕行"。这条报错来自 `lib/hls-pipeline.js#downloadHlsSegments` 的 `HLS_OUTPUT_TOO_LARGE` 守卫——**设计如此**：与其产出截断文件，不如明确中止。但它触发的根因是我们的取舍：HLS 变体若带独立音轨（`EXT-X-MEDIA:TYPE=AUDIO` + 变体 `audioGroupId`），合并时必须整体持有视频数据（fMP4 muxer 接口所限），于是 `background/hls-fetcher.js` 会关掉 OPFS 流式落盘、上限退回 `MAX_IN_PAGE_MERGE_BYTES(2GB)`；1.7GB 视频 + 音轨合并后跨过 2GB 就中止了。改进：`lib/hls-pipeline.js` 新增 `pickVariantForMemoryLimit()`（纯函数+4 条用例）——下载前若"需要音轨合并"且预估体积超上限，就自动改选**预合并变体**（无 AUDIO 组）里码率最高的一条，从而走 sink/OPFS 流式落盘（上限 8GB）；`hls-fetcher` 在改选后重新计算 `audioMergePlanned`（预合并变体不再需要 muxer）。没有预合并变体时仍按原上限明确中止（提示换更低清晰度或录制模式），不会产出损坏文件。全量 693 项通过。 |
| 1.17.30 | 2026-09-25 | 修复现场「1.7GB 的视频报 Array buffer allocation failed」。根因：自适应流合并要在内存里同时持有**两路输入 + 合并输出 + Blob 物化**（峰值 ≈3~4× 总字节），而原先的合并上限沿用 `MAX_IN_PAGE_MERGE_BYTES = 2GB`——1.7GB 的输入刚好在限内被放行，分配到一半就炸；更糟的是估算只看 `contentLength`，visionOS 这类响应缺该字段时估算为 0，连 2GB 这条线都绕过了。修复：①新增常量 `YOUTUBE_MERGE_MAX_BYTES = 768MB`（后台与内容侧统一）；②`background/download-strategies/youtube-adaptive-download-strategy.js` 新增纯函数 `decideAdaptiveDownloadRoute()`：限内 → 内存合并；超限且有 HLS 清单 → **自动改走 HLS 流式落盘**（`HlsFetcher.downloadAndMerge`，内部按体积切 OPFS，上限 8GB，不再经过页面纯内存路径）；超限且无 HLS → 明确报错而不是 OOM；③估算在 `contentLength` 缺失时回退 `bitrate×时长`，并给两路抓取加运行期上限（并行路径用 range 探针拿到的真实长度、resumable 路径累计到上限即中止且不重试，错误码 `YOUTUBE_MERGE_TOO_LARGE`）；④内容侧同一上限，报错文案改为"请改选「HLS 预合并」清晰度或更低清晰度，也可用录制模式"。新增用例 4 条，全量 689 项通过。 |
