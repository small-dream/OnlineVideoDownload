...
| 1.17.33 | 2026-09-26 | 修复现场「下载得到 `Learn 10 phrases …_2160p.mp4.txt`」。成因：把媒体直链交给浏览器下载管理器后，googlevideo 拒绝请求并回 **text/plain 错误页**，Chrome 按 MIME 给文件名追加扩展名 → `xxx.mp4.txt`（内容是 403 页，不是视频）。三道防线：①`background/download-strategies/youtube-adaptive-download-strategy.js` 在提交下载管理器前做**单字节 Range 预检**（`probeDirectStream` + 纯函数 `classifyDirectProbe`：4xx/5xx 或 text/xml 即判定为错误页），被拒时**不提交**，而是先用 `preferCombined:false` 重新选流改走合并/流式合并路径（有自适应流就能正常下 2160p），实在没有才抛出可操作错误；探测本身失败（CORS/网络）不拦截，避免误伤。②新增 `lib/download-artifact-utils.js#isBrokenTextStubDownload`：SW 在 `state=complete` 时校验产物（`.txt` 后缀，或文本 MIME 且 <512KB），命中即 `removeFile`+`erase` 删除残片、任务标记 failed 并弹失败通知——即使别的路径漏了预检也不会再留下"假成功"的 .txt。③新增用例 3 条（残片识别、探测判定），全量 690 项通过。 |
| 1.17.32 | 2026-09-26 | 回退 1.17.30 / 1.17.31 的两个"绕行"补丁（超限就改走 HLS、改选预合并变体），改为**彻底解决：流式合并落盘**。①新增 `lib/fmp4-file-merge.js`：稀疏遍历顶层盒子（只读 8~16 字节盒头）+ 只读 moov（小）解析解码配置/时长基准/宽高 + 逐 moof 解析样本描述（`parseFragment(..., {descriptorsOnly:true})`，不读媒体数据）+ 按片段批量读样本喂 `Mp4Muxer`，内存峰值 ≈ 一个 moof + 一个片段，与文件体积解耦。②新增 `background/streaming-merge.js`：按 Range 分块下载（单分块内存、失败重试、服务器忽略 Range 时退化为整段流式）→ 顺序写入 OPFS；文件级合并用 `Mp4Muxer.StreamTarget({chunked:true,onData})` 按绝对偏移写进第三个 OPFS 文件（`createOpfsSink` 新增 `writeAt(position, chunk)`，onData 的 Promise 会收集后统一 await，避免关闭文件时丢尾部/moov）；最后 `submitOpfsDownloadFromOffscreen` 保存并按 downloadId 登记临时文件清理。③两处接入：YouTube 自适应流预估 > `STREAM_MERGE_THRESHOLD_BYTES`(512MB) 走流式（现场 1.7GB 视频不再抛 `Array buffer allocation failed`）；HLS 独立音轨合并预估 > 阈值时，视频分片与音轨各自落盘再文件级合并（替代原来"撞 2048MB 上限就中止"）。④页面上下文（纯内存）上限同步到 512MB，错误文案指向"在弹窗里下载（会走流式合并落盘）"。⑤`lib/bilibili-muxer`：`parseFragment` 支持 `descriptorsOnly`、`__internals` 补导出解码配置解析件。新增用例 2 条（分块落盘、端到端文件级合并产出双轨可用 MP4），全量 687 项通过。 |
| 1.17.29 | 2026-09-25 | 修复现场反馈「合并后的视频声音不是原视频的」。根因：带**自动配音**的视频，YouTube 会在同一份 `adaptiveFormats` 里返回多条音频轨（现场 visionOS 客户端给了 **65 条 audio**），而 `pickAdaptiveAudioStream` 只按 `bitrate` 取最高 —— 命中的往往是配音轨（如 Hindi/Portuguese），合进视频后自然"对不上"。修复：①`injected/page-youtube-parser.js` 的音频流映射补 `audioTrackId` / `audioTrackIsDefault`（`adaptiveFormats[].audioTrack.audioIsDefault`）/ `audioTrackName`；②`lib/youtube-stream-utils.js` 新增 `isOriginalAudioTrack`（优先 `audioIsDefault`，缺字段时按名称含 `original`/`原声` 兜底），`pickAdaptiveAudioStream` 改为"先筛原声轨、再在轨内按码率取最高"，无音轨信息时保持原行为；③`youtube-adaptive-download-strategy#mergeAdaptiveStreams` 增加选流日志（`audioItag/audioTrack/audioDefault`）便于现场核对。content 侧解析策略共用同一 picker，两条路径同时生效。新增用例 3 条，全量 685 项通过。 |
| 1.17.28 | 2026-09-25 | 修复现场反馈「设置里选了『解析下载』，下载却提示正在录制视频」。根因是**两个设置 store 从未打通**：设置 UI 写的是 `generalSettings.youtubeDefaultMode`（`popup.js` 的 `popupYoutubeDefaultMode`），而下载路径读的是 `youtubeDownloadPrefs.mode`（`lib/youtube-download-mode-store.js`，由 `content/youtube-download-options.js#normalizeYouTubeDownloadOptions` 取值）。用户在设置里切到解析下载后，下载侧拿到的仍是 `capture` → `getExecutionMode` 走 content → 内容侧 youtube handler 按 `mode !== 'parse'` 选中**录制策略**，于是提示"正在实时录制 YouTube 视频"。修复：①`popup.js` 在 `loadPopupSettings`（popup 每次打开都会跑）里把 `youtubeDefaultMode` 同步进 `youtubeModeStore`，并在设置项 change 时双写，两者保持一致；②顺带加固解析/合并路径的网络层：`youtube-adaptive-download-strategy` 的视音频抓取改为 `credentials:'include'`，并给 `injectHeaders` 传 `corsOrigin = chrome.runtime.getURL('')`，让 DNR 回显扩展来源的 CORS 头（`credentials:'include'` 下 `ACAO: *` 会被浏览器拒绝）——visionOS 这类"设备客户端"给的地址可能与会话绑定，需要带上浏览器 Cookie。全量 682 项通过。 |
| 1.17.27 | 2026-09-25 | 真正修好「只能选默认最高清晰度」。现场日志给出了决定性线索：上报的清单地址是 `https://manifest.googlevideo.com/api/manifest/hls_variant/...`，而 `isYouTubeHlsEntry` 当初只匹配 `hls_playlist` —— **master 清单（hls_variant）没被认出来 → `mergeYouTubeHlsEntries` 合并失败 → 过滤上下文里仍带着 HLS 条目 id → `shouldHideRedundantDetection` 把带"（需合并）"清晰度列表的 youtube 行整行隐藏**，只剩 HLS 行的"自动（最高画质）"。修复：识别改为 `/api/manifest/hls[_a-z0-9]*/`（同时覆盖 `hls_variant` 与 `hls_playlist`）并补两条用例（variant 形态的识别与合并）。同一份日志还确认了客户端选择的正确性：TVHTML5 回 `UNPLAYABLE 需要重新加载页面`、web_embedded 回 `ERROR 此视频不能观看`、web_safari 回 `UNPLAYABLE 视频无法播放`，**visionOS 客户端成功**（`formats=87 maxDirectHeight=2160 directVideo=22 directAudio=65 hls=yes`），因此 `pickBestClientResult` 选它是对的。全量 682 项通过。 |
| 1.17.26 | 2026-09-25 | 现场日志确认数据层已经打通：`direct client ok key=visionos formats=87 maxDirectHeight=2160 directVideo=22 directAudio=65 hls=yes`（TVHTML5 回 `UNPLAYABLE 需要重新加载页面`、web_embedded 回 `此视频不能观看`、web_safari 回 `UNPLAYABLE 视频无法播放`，visionOS 客户端成功），并已 `reporting … source=direct-client` + `report HLS manifest`。用户仍看到"只有默认最高清晰度"是因为 service worker / 内容脚本仍是旧版本（1.17.23 的合并与 1.17.25 的过滤顺序修复都在 SW 侧）。本轮另修两处细节：①`normalizeResolutionForVideo` 对 `hls:*` 值增加"该视频没有 HLS 清单时退回普通清晰度逻辑"的分支，避免偏好被记住后换视频出现悬空值；②popup 的 HLS 变体获取失败时补日志（`YouTube HLS 变体获取失败` / `HLS 画质获取失败` + URL），便于区分"没拉到"与"没注入"。全量 682 项通过。 |
| 1.17.25 | 2026-09-25 | 修复现场反馈「`需合并` 选不了，只剩默认最高分辨率」。根因是 1.17.23 引入记录合并后，`getVisibleVideosForTab` **仍用合并前的列表**构造过滤上下文：被吸收掉的 YouTube HLS 条目 id 还在 `youtubeHlsVideoIds` 里，于是 `shouldHideRedundantDetection` 把合并后那条 `youtube-adaptive`（也就是带"（含音频）/（需合并）"清晰度与模式入口的那行）整行隐藏，只剩 HLS 行 → 下拉里只有"自动（最高画质）"。修复：先 `mergeYouTubeHlsEntries` 再 `buildDetectionFilterContext`（顺序在代码里注释说明），并补一条顺序回归用例。顺带修掉"HLS 变体列表拉不出来"：`content/strategies/hls-strategy.js#fetchQualities` 之前是裸的内容脚本 fetch，对 YouTube 这类跨域 CDN 会因 Origin/CORS 被拒（下拉只剩"自动"），现在与下载路径一致——先借 background 的 DNR 规则注入 Referer/CORS（`injectRequestHeaders`，由 `content-main` 传入 `injectDownloadHeaders`），结束后清理。新增用例 1 条，全量 682 项通过。 |
| 1.17.24 | 2026-09-25 | 修复现场报错「First argument to DataView constructor must be an ArrayBuffer」（下载进度走完、进入合并阶段时报）。根因：下载管线"拼接分片"用的是 `byteUtils.concatUint8Arrays`（返回 **Uint8Array**），而 MP4 解析入口是 `new DataView(buffer)`（**只接受 ArrayBuffer**）——两条路径在"视频流 ≥ `YOUTUBE_PARALLEL_MIN_BYTES`(16MB) 时走并行分片抓取"的情况下相撞；小文件走 resumable 路径返回的是 ArrayBuffer，所以此前单测（几百字节 fixture）没暴露，44MB 的真实视频正好命中。修复：①`lib/byte-utils.js` 新增 `toArrayBuffer`（ArrayBuffer 直通、TypedArray/DataView 按 `byteOffset+byteLength` 还原成独立 buffer、非法输入返回 null），并纳入单测；②`lib/bilibili-muxer.js` 在 `mergeBilibiliDash`（合并入口）、`parseFragment`、`parseBoxes` 三处统一归一化，输入为空时抛出明确错误「视音频数据为空或类型不支持（需要 ArrayBuffer 或 Uint8Array）」而不是底层 DataView 报错；③`lib/hls-pipeline.js#toMuxBuffer` 也改走该工具（此前传 ArrayBuffer 会落到 `bytes.buffer.slice` 抛错）。新增用例 2 条（Uint8Array/带 byteOffset 视图的端到端合并、`toArrayBuffer` 单测），全量 681 项通过。 |
