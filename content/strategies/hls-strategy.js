...
(() => {
  ...
  function createHlsDelegateHandler(options = {}) {
    ...
    async function handle(m3u8Url, filename, headers, taskMeta = {}) {
      console.log(`[OVD] HLS 委托下载开始 url=${m3u8Url} filename=${filename}`);
      const sourceUrl = m3u8Url;

      let m3u8Text = await hlsPipeline.hlsFetchText(m3u8Url, headers);
      ...
      if (m3u8Text.includes('#EXT-X-STREAM-INF')) {
        ...
        m3u8Text = await hlsPipeline.hlsFetchText(mediaUrl, headers);
        m3u8Url = mediaUrl;
      }

      const keyInfo = await hlsPipeline.parseHlsEncryption(m3u8Text, m3u8Url, headers);
      if (keyInfo) {
      ...
      for (let i = 0; i < segments.length; i += concurrency) {
        ...
        const results = await Promise.all(
          batch.map((url, index) => hlsPipeline.hlsFetchBuffer(url, headers)
      ...
      return { downloadId: null, segmentCount: segments.length };
    }
