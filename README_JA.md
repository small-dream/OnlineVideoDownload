...
- 🏷️ **バッジカウンター** — ツールバーアイコンに現在のタブで検出された動画数がひと目でわかります
- ☑️ **バッチダウンロード** — 複数選択 / 全選択で、複数の動画をワンクリックで同時ダウンロード
...
- ページに実際に動画コンテンツがあることを確認してください（v1.16.0 より iframe 埋め込み動画の検出に対応しています。CSP が厳格なページでは注入がブロックされる場合があります）
