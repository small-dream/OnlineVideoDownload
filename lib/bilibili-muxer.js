 ...
 */
function parseFragment(buffer, fragment) {
  const samples = [];
  const { moofPayloadOffset, moofPayloadSize, mdatPayloadOffset } = fragment;
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    samples.push({
      ...
      data: new Uint8Array(buffer, mdatPayloadOffset + dataOffset, size),
      duration,
      isKey,
      ctOffset,
    });
...
var BilibiliMuxer = {
  ...
  __internals: { collectFragments, findBox, parseBoxes, parseFragment },
  mergeBilibiliDash,
