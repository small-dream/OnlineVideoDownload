...
function parseBoxes(buffer, offset = 0, end = null) {
  ...
  while (pos + 8 <= end) {
    ...
    const boxData = buffer.slice(pos + 8, pos + size);
    if (!boxes.has(type)) {
      boxes.set(type, { offset: pos, size, data: boxData });
    } else {
      // 允许多个同名盒子（moof, mdat）存为数组
      const existing = boxes.get(type);
      if (!Array.isArray(existing)) {
        boxes.set(type, [existing, { offset: pos, size, data: boxData }]);
      } else {
        existing.push({ offset: pos, size, data: boxData });
      }
    }
    pos += size;
  }
 ...
 */
function parseFragment(moofBuffer, mdatBuffer) {
  const samples = [];
  const moofBoxes = parseBoxes(moofBuffer);
  const traf = moofBoxes.get('traf');
  if (!traf) return samples;

  const trafBoxes = parseBoxes(moofBuffer, traf.offset + 8, traf.offset + traf.size);
  const tfhd = trafBoxes.get('tfhd');
  const trun = trafBoxes.get('trun');
  ...
  if (tfhd) {
    const tfhdDv = new DataView(moofBuffer, tfhd.offset + 8);
    const tfhdFlags = (tfhdDv.getUint8(1) << 16) | (tfhdDv.getUint8(2) << 8) | tfhdDv.getUint8(3);
    let tfhdPos = 8; // skip version(1)+flags(3)+track_id(4)
    if (tfhdFlags & 0x000001) tfhdPos += 8; // base-data-offset
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    if (size === 0) {
      muxerWarn('[BilibiliMuxer] 样本 size 为 0，跳过');
      continue;
    }

    // 判断是否关键帧（flags: bit 25 表示非同步样本，0 = 关键帧）
    const isNonSync = (flags >> 16) & 0x0001;
    const isKey = !isNonSync;

    const sampleData = new Uint8Array(mdatBuffer, dataOffset, size);
    samples.push({
      data: new Uint8Array(sampleData), // 复制一份，避免持有 mdat 引用
      duration,
      isKey,
      ctOffset,
    });
...
async function mergeBilibiliDash(videoBuffer, audioBuffer, onProgress) {
  ...
  // 3. 收集所有 moof+mdat 对
  function collectFragments(buffer, boxes) {
    const fragments = [];
    const allBoxes = [];
    const view = new DataView(buffer);
    let pos = 0;
    ...
    for (let i = 0; i < allBoxes.length; i++) {
      if (allBoxes[i].type === 'moof') {
        ...
        if (mdat && mdat.type === 'mdat') {
          fragments.push({
            moofData: buffer.slice(moof.offset + 8, moof.offset + moof.size),
            mdatData: buffer.slice(mdat.offset + 8, mdat.offset + mdat.size),
          });
          i++; // 跳过 mdat
        }
      }
  ...
  for (const frag of videoFragments) {
    const samples = parseFragment(frag.moofData, frag.mdatData);
  ...
  for (let i = 0; i < videoSamplesWithTs.length; i++) {
    ...
    try {
      ...
      muxerWarn(`[BilibiliMuxer] 视频样本 ${i} 写入失败: ${err.message}`);
    }
    processedSamples++;
    if (i % 100 === 0) {
  ...
  for (let i = 0; i < audioSamplesWithTs.length; i++) {
    ...
    try {
      ...
      muxerWarn(`[BilibiliMuxer] 音频样本 ${i} 写入失败: ${err.message}`);
    }
    processedSamples++;
    if (i % 200 === 0) {
      onProgress?.(30 + Math.round((processedSamples / totalSamples) * 60));
