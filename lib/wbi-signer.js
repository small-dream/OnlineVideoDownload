...
(() => {
  if (globalThis.__OVD_WBI_SIGNER__) {
    return;
  }
  ...
  async function calcWrid(params, imgKey, subKey) {
    ...
    const queryStr = new URLSearchParams(sortedParams).toString() + mixinKey;
    const encoder = new TextEncoder();
    const data = encoder.encode(queryStr);
  ...
  function md5(str) {
    ...
    const nblocks = Math.ceil((bytes.length + 8) / 64);
    const words = new Uint32Array(nblocks * 16);
    for (let i = 0; i < bytes.length; i++) {
      words[i >> 2] |= bytes[i] << ((i % 4) * 8);
    }
    words[bytes.length >> 2] |= 0x80 << ((bytes.length % 4) * 8);
    words[nblocks * 16 - 2] = bytes.length * 8;
