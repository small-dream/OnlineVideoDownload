...
// 浏览器矩阵：本项目仅支持 Chromium 内核（Chrome / Edge 109+）。
// 原因见 docs/ARCHITECTURE.md「浏览器矩阵」：offscreen document、
// declarativeNetRequest 动态规则、scripting.executeScript({ world: 'MAIN' })、
// runtime.getContexts 都是 Chromium 专有或语义差异过大，Firefox 的等价方案
// 需要重写离线下载与页面注入两条链路，因此不做「看似兼容」的探测。
...
(() => {
  function detectBrowser() {
    ...
    const isChromium = isChrome || isEdge || isOpera || /Chromium\//.test(ua);
    ...
    return {
      ...
      isEdge,
      isOpera,
      name: isEdge ? 'Edge' : isChrome ? 'Chrome' : isOpera ? 'Opera' : (isChromium ? 'Chromium' : 'Unsupported'),
