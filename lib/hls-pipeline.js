...
(() => {
  ...
  /**
   * 统一密钥解析入口（content 与 background 共用）。
   * 有 EXT-X-KEY 列表时返回 { keys, segments } 支持轮换；
   * 仅标记加密但无密钥条目时退回单密钥 parseHlsEncryption。
   */
  async function resolveHlsKeyInfo(playlist, playlistText, playlistUrl, headers, options = {}) {
    const keyEntries = playlist?.keys || [];
    if (keyEntries.length > 0) {
      return {
        keys: await resolveHlsKeys(keyEntries, headers, options),
    ...
    return parseHlsEncryption(playlistText, playlistUrl, headers, options);
  }

  /**
   * muxer 需要 ArrayBuffer；视图已覆盖整个 buffer 时直接复用，避免再复制一份全量数据。
   */
  function toMuxBuffer(bytes) {
    const byteUtils = globalThis.__OVD_BYTE_UTILS__ || {};
    if (typeof byteUtils.toArrayBuffer === 'function') {
      return byteUtils.toArrayBuffer(bytes);
    }
    if (!bytes) {
      return null;
    }
  ...
  async function downloadHlsSegments(segments, options = {}) {
    ...
    // 落盘是串行的（单个 writable / 顺序文件），但不该把下载挡在后面：
    // 写入放到一条串行链上跑，只有"未落盘分片数"超过窗口时才回压等待。
    const sinkWindow = Number(options.sinkWindow) > 0 ? Number(options.sinkWindow) : (options.concurrency || constants.HLS_SEGMENT_CONCURRENCY || 5);
    ...
    /** 把本轮可写内容排到串行写入链上（不阻塞下载）；错误由 writeError 统一上报 */
    function scheduleSinkFlush() {
    ...
    async function drainSinkIfNeeded() {
      if (!pending || pending.size <= sinkWindow) {
        return;
      }
      await writeChain;
      if (writeError) {
    ...
    if (pending) {
      // 收尾：等最后一批写入落地，并透出写入/解密错误（不产出半截文件）
      await writeChain;
      if (writeError) {
        throw writeError;
      }
  ...
  /**
   * 内存上限内的降级选择：需要"独立音轨合并"的变体必须整体持有视频数据
   * （fMP4 muxer 接口所限），因此体积超限时不能走 OPFS 流式落盘，只能中止。
   * 这里在下载前判断一次：若超限且存在**预合并**（没有 AUDIO 组）的变体，
   * 就改选其中码率最高的那条，让它走 sink/OPFS 路径。
   *
   * @returns {{changed: boolean, muxed: Object|null, variant: Object}}
   */
  function pickVariantForMemoryLimit(master, variant, { estimatedBytes = 0, limitBytes = 0 } = {}) {
    const needsAudioMerge = !!findMatchingAudioRendition(master, variant);
    const overLimit = Number(limitBytes) > 0 && Number(estimatedBytes) > Number(limitBytes);
    if (!needsAudioMerge || !overLimit) {
      return { changed: false, muxed: null, variant };
    }

    const muxed = (master?.variants || [])
      .filter((candidate) => candidate?.url && !candidate.audioGroupId)
      .slice()
    ...
    return { changed: true, muxed, variant: muxed };
  }
